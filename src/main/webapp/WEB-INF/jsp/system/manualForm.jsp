<%@ page language="java" errorPage="/sample/common/error.jsp" pageEncoding="UTF-8" contentType="text/html;charset=UTF-8" %>
<%@ include file="/taglib/taglibs.jsp"%>
<script type="text/javascript">
	onMenu(3);
</script>
<div class="tit_box">사용자 매뉴얼</div>
    <div class="bt_tab_box01">
   	<div class="bt_tab_txt01">사용자매뉴얼</div>
       <div class="bt_tab_txt01" style="color:#858686"><a href="<c:url value='/system/codeForm.do' />">코드관리</a></div>
   	</div>
	<div class="bg_formbox" style="height:517px;"> <!-- form배경 박스 높이를 517로 늘임 -->
    	<div class="textbox">
        	<div class="textbox_tit">마이그레이션 엔진 개요</div>
        	<div class="textbox_sub">본 마이그레이션 엔진은 WebLogic, Jeus 등의 상용 애플리케이션 서버를 공개 SW기반의 애플리케이션 서버에 디플로이 될 수 있는 형태로 만들어 주는 도구입니다.</div>          
        </div> 
        <div class="textbox02">
        	<div class="textbox_tit">로그인</div>
        	<div class="textbox_sub">본 마이그레이션 엔진은 WebLogic, Jeus 등의 상용 애플리케이션 서버를 공개 SW기반의 애플리케이션 서버에 디플로이 될 수 있는 형태로 만들어 주는 도구입니다.</div>          
        </div> 
        <div class="textbox02">
        	<div class="textbox_tit">마이그레이션 프로젝트 입력 페이지</div>
        	<div class="textbox_sub">본 마이그레이션 엔진은 WebLogic, Jeus 등의 상용 애플리케이션 서버를 공개 SW기반의 애플리케이션 서버에 디플로이 될 수 있는 형태로 만들어 주는 도구입니다.</div>          
        </div>      
    </div>
